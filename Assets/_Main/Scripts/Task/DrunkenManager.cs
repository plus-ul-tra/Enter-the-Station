...
public class DrunkenManager : Task
{
    ...
    void Update()
    {
        ...
        if (isReached)  // 스마일 아이콘
        {
            ...
            pressSign.gameObject.SetActive(true);
        }
        else if (!isReached)
        {
            //if (isLeft)
            ...
            pressSign.gameObject.SetActive(false);
        }
        ...
        if (Input.GetKeyDown(KeyCode.Space) && !isOver)
        {
            ...
            else if (!isReached)
            {
                ...
                baseLineGroup.transform.GetChild(countLevel).gameObject.SetActive(false);
                //countLevel--; BIC 유저 테스트 후 주석 처리함. 2025.08.16 BSJ
                baseLineGroup.transform.GetChild(countLevel).gameObject.SetActive(true);
            }
        }
    ...
    //       .Join(img.DOFade(0f, 0.2f)) // 동시에 투명도 감소
    ...
    //           // 재사용을 위한 복원
    //           img.color = new Color(img.color.r, img.color.g, img.color.b, 1f); // 색상(알파) 복원
